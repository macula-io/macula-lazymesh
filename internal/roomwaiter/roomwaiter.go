...
// macula-io/macula-lazymesh#10/#13/#14). Built for #14's spike; not
// wired into cmd/lazymesh's production path.
package roomwaiter
...
func (m *Manager) enqueue(room string) {
	...
	default:
		// 64 distinct rooms already pending at once -- drop rather than
		// block a waiter goroutine forever. pending stays set, so this
		// room is not silently re-queued either; a real design (not this
		// spike) would need a bigger buffer or a slower path here, but 64
		// concurrent pending rooms is already past what this spike's own
		// measurements are checking for.
	}
}
